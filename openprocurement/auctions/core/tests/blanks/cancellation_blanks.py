...
def create_auction_cancellation(self):
    ...
    self.assertEqual(response.json['errors'][0]["description"],
                     "Can't add cancellation in current (cancelled) auction status")
...
def patch_auction_cancellation(self):
    ...
    self.assertEqual(response.content_type, 'application/json')
    self.assertEqual(response.json['errors'][0]["description"],
                     "Can't update cancellation in current (cancelled) auction status")

    response = self.app.patch_json('/auctions/{}/cancellations/some_id'.format(self.auction_id),
    ...
    response = self.app.patch_json('/auctions/some_id/cancellations/some_id', {"data": {"status": "active"}},
                                   status=404)
    self.assertEqual(response.status, '404 Not Found')
    self.assertEqual(response.content_type, 'application/json')
    self.assertEqual(response.json['status'], 'error')
