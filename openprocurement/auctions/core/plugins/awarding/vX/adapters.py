...
class AwardingVXConfiguratorMixin(BaseAwardingMixin):
...
class AwardingNextCheckVX(AuctionAwardingNextCheckAdapter):
    """Use next_check_awarding from V3 awarding"""
    def add_awarding_checks(self, auction):
        return next_check_awarding(auction)


class AwardManagerVXAdapter(BaseAwardManagerAdapter):

    name = 'Award v-X adapter'
